<?php
class zyme_widget_recent_photos extends WP_Widget {
    ...
    function __construct(){
        parent::__construct(
            'recent_photos',
            'zyme近期图像',
            array( 'description' => __( '根据设置可以显示不同数量的图片', 'zyme' ) )
        );
    ...
    function widget($args, $instance){
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? __('近期图像','zyme') : $instance['title']);//小工具前台标题
        ...
        if( $title ) echo $args['before_title'] . '<i class="zyme ' . $icon . '"></i> ' . $title . $args['after_title'];
        if (function_exists('zyme_recent_photos')) { echo "<ul class='widget-photos'>" . zyme_recent_photos($phoNum) . "</ul>";} ;
        echo $after_widget;  //框架结束
    }
}
register_widget('zyme_widget_recent_photos');