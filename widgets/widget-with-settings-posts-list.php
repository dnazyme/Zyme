 ...
 * 小工具：文章
 */
class zyme_widget_posts_list extends WP_Widget{
	...
	function __construct(){
		parent::__construct(
			'posts_list',
			'zyme文章列表' . __( '', 'zyme' ),
			array( 'description' => __( '根据设置可以显示不同的文章', 'zyme' ) )
		);
	...
	function widget( $args, $instance ){
		...
		echo $args['before_widget'];
			if( !empty( $title ) ) echo $args['before_title'] . '<i class="zyme ' . $icon . '"></i> ' . $title . $args['after_title'];
			...
			zyme_sidebar_posts_list( $query_args );
	...
	function form( $instance ){
		...
		$orderby = array(
			'date'          => __( '发布时间', 'zyme' ),
			'comment_count' => __( '评论数量', 'zyme' ),
			'post_views_count'         => __( '浏览次数', 'zyme' ),
			'rand'          => __( '随机排列', 'zyme' )
		);

		$date_limit = array(
			'unlimited'   => __( '无限制', 'zyme' ),
			'1 day ago'   => __( '一天之内', 'zyme' ),
			'3 day ago'   => __( '三天之内', 'zyme' ),
			'1 week ago'  => __( '一周之内', 'zyme' ),
			'1 month ago' => __( '一个月之内', 'zyme' ),
			'3 month ago' => __( '三个月之内', 'zyme' ),
			'6 month ago' => __( '半年之内', 'zyme' ),
			'1 year ago'  => __( '一年之内', 'zyme' ),
			'2 year ago'  => __( '两年之内', 'zyme' ),
			'3 year ago'  => __( '三年之内', 'zyme' )
		);
?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( '标题：', 'zyme' ); ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'icon' ) ); ?>"><?php _e( '图标：', 'zyme' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php _e( '文章排序：', 'zyme' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'descending' ) ); ?>"><?php _e( '倒序排列：', 'zyme' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( '文章数量：', 'zyme' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'date_limit' ) ); ?>"><?php _e( '日期限制：', 'zyme' ); ?></label>
		...
		?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'exclude_tax' ) ) . "[$tax_name]"; ?>"><?php printf( __( '排除%s：', 'zyme' ), $tax->labels->name ); ?></label>
...
register_widget( 'zyme_widget_posts_list' );
