						...
						<?php if( $post->post_type== 'post' ) { ?>
			                <li class="zyme-item">
			                    <article class="post post-type">
									<?php if( is_sticky() ) _e('<div class="set-top"><div>置顶</div></div>','zyme'); ?>
									<header class="post-type-header">
										...
										<?php } else { ?>
											<div class="thumbnail-link" style="background-image:url('<?php $unique_id = cs_get_option('zyme_post_image'); $attachment = wp_get_attachment_image_src( $unique_id, 'full' ); $image_url = ($attachment) ? $attachment[0] : $unique_id; print_r($image_url);?>')"></div>
										<?php } ?>
										<div class="post-relative">
											<?php echo get_avatar( get_the_author_meta( 'ID' ) ); ?>
											<span class="post-type-author"><?php echo the_author_posts_link(); ?> <i class="zyme zyme-certify"></i><span class="publish-time"> - <?php echo human_time_diff(get_the_time('U'), current_time('timestamp')); _e('前'); ?></span>
											</span>
									...
									<div class="post-main post-type-main">
			                            <div class="post-content">
											...
											<div class="post-excerpt"><?php zyme_excerpt( 160, '……' ); ?></div>
											<div class="post-category"><?php _e('来自分类：');the_category( ' / ' ); ?></div>
			                            </div>
			                        </div>
			                        <div class="post-footer">
		                                <span class="post-view">
		                                    <i class="zyme zyme-view"></i>&nbsp;<?php echo getPostViews(get_the_ID()); ?>
		                                </span>
		                                <span class="post-comments">
		                                    <i class="zyme zyme-comment"></i>&nbsp;<?php if ( post_password_required() ) { echo '<a>密码保护</a>'; } else { comments_popup_link('0', '1', '%', '', '评论已关闭'); } ?>
		                                </span>
		                                <span class="post-like">
											<a href="javascript:;" data-action="zyme_like" data-id="<?php the_ID(); ?>" class="like<?php if(isset($_COOKIE['zyme_like_'.$post->ID])) echo ' have-like';?>">
												<span class="like-count"><?php if( get_post_meta($post->ID,'zyme_like',true) ){ echo get_post_meta($post->ID,'zyme_like',true); } else { echo '0'; }?></span>
    										</a>
										</span>
		                                <span class="post-readmore">
		                                    <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
		                                        <i class="zyme zyme-more-o"></i> 阅读
		                                    </a>
		                                </span>
			                        </div>
			                    </article>
						...
						<?php } else if( $post->post_type== 'shuoshuo' ) { ?>
			                <li class="zyme-item">
			                    <article class="shuoshuo post-type">
									<?php if( is_sticky() ) _e('<div class="set-top">置顶</div>','zyme'); ?>
									<header class="post-type-header">
										<div class="post-relative">
											<?php echo get_avatar( get_the_author_meta( 'ID' ) ); ?>
											<span class="post-type-author"><?php echo the_author_posts_link(); ?> <i class="zyme zyme-certify"></i><span class="publish-time"> - <?php echo human_time_diff(get_the_time('U'), current_time('timestamp')); _e('前'); ?></span>
									...
									<?php echo get_avatar( get_the_author_meta( 'ID' ) ); ?>
										...
										<div class="art-info">
			                                <span class="art-info-date">
			                                    <i class="zyme zyme-calendar"></i>&nbsp;<?php echo human_time_diff(get_the_time('U'), current_time('timestamp')) . '前'; ?>
											</span>&nbsp;•&nbsp;
											<span class="art-info-view">
			                                    <i class="zyme zyme-view"></i>&nbsp;<?php echo getPostViews(get_the_ID()); ?>
			                                </span>&nbsp;•&nbsp;
			                                <span class="art-info-comment">
			                                    <i class="zyme zyme-comment-1"></i>&nbsp;<?php comments_popup_link('0', '1', '%', '', '评论已关闭'); ?>
			                                </span>
			                            </div>
									</header>
									<div class="art-main">			                            
			                            ...
			                            <div class="article-info info-index">
											<span class="post-like">
         										<a href="javascript:;" data-action="ding" data-id="<?php the_ID(); ?>" class="favorite<?php if(isset($_COOKIE['zyme_ding_'.$post->ID])) echo ' done';?>"><span class="count">
           										<?php if( get_post_meta($post->ID,'zyme_ding',true) ){
                    								echo get_post_meta($post->ID,'zyme_ding',true);
                 								} else {
                    								echo '0';
                 								}?></span>
        										</a>
			                                ...
			                                <span class="art-info-readmore">
			                                    <a href="<?php the_permalink(); ?>">
			                                        <i class="zyme zyme-more-o"></i> 阅读全文
			                                    </a>
			                                </span>
			                            </div>
			                        </div>
