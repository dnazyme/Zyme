...
App = {
    mouseEvent: function () {
        ...
        $(document).on('click', '#menu-bar.zyme-menu', function () {
            $('#menu-bar').animate({ left: -42 }, 200, function () {
                $(this).removeClass('zyme-menu').addClass('zyme-close');
                $(this).animate({ left: 0 }, 200);
            });
            $('#menu-login').animate({ right: -42 }, 200);
            $('#mobile-menu').fadeIn(200).css('top', 0);
        });
        $(document).on('click', '#menu-bar.zyme-close', function () {
            $('#menu-bar').animate({ left: -42 }, 200, function () {
                $(this).removeClass('zyme-close').addClass('zyme-menu');
                $(this).animate({ left: 0 }, 200);
        ...
        $(document).on('click', '.post-like a', function () {
            if ($(this).hasClass('have-like')) {
                ...
                $.post(zymeConfig.ajaxUrl, ajax_data, function (data) {
        ...
        // 复制
        $(document).on('click', 'a.social-share-icon.zyme-copy', function () {
        ...
        $(document).delegate('.pay-header .zyme-close', 'click', function (e) {
            e.stopPropagation();
            $('#layout-shadow').hide();
            ...
            if (href != undefined) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var $result = $(data).find(".posts-list .zyme-item");
    ...
    imageLazyLoad: function () {
        $("div.lazy").lazyload({
            placeholder: zymeConfig.siteUrl + "/img/loading.gif",
            effect: "fadeIn",
        });
    },
    ...
    owoEmoji: function () {
        $('.OwO').each(function (i, block) {
            var s = new OwO({
                logo: '<i class="zyme zyme-emoji"></i>',
                container: document.getElementsByClassName('OwO')[0],
                target: document.getElementsByClassName('error')[0],
                position: 'down',
                width: '100%',
                maxHeight: '200px',
                api: zymeConfig.siteUrl + "/emoji/OwO.min.json"
            });
        });
    },

    commentsSubmit: function () {
        var edit_again = zymeConfig.commentEditAgain,
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ((parent == '0')) {
                        ...
                        $('ol.zyme-comments-area').prepend(new_item);
                    } else {
        ...
        if (getVersion(zymeConfig.version == "5.1") >= 510) {
            console.log("wp5.1+");
            // 修复wp5.1评论回复bug
    ...
    commentsPaging: function () {
        $('body').on('click', '.zyme-comments-page a.page-numbers', function (e) {
            e.preventDefault();
            $.ajax({
                type: 'GET',
                url: $(this).attr('href'),
                beforeSend: function () {
                    $('ol.zyme-comments-area').html('');
                    loading_start($('ol.zyme-comments-area'));
                },
                dataType: 'html',
                success: function (out) {
                    result = $(out).find('ol.zyme-comments-area');
                    nextlink = $(out).find('.zyme-comments-page');
                    $('#comments').html(result);
                    $('ol.zyme-comments-area').append('<div id="pagination"></div>');
    ...
    startTime: function () {
        function show_date_time() {
            ...
            var blogStartTime = zymeConfig.siteStartTime;
