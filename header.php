...
<head>
	...
	$keywords = '';
	if (is_home()) {
		$description = cs_get_option('zyme_description');
		$keywords = cs_get_option('zyme_keywords');
	} elseif (is_single()) {
		$description1 = zyme_meta($post->ID, '_zyme_post_options', 'post_description');
		...
		$keywords = zyme_meta($post->ID, '_zyme_post_options', 'post_keywords');
		if ($keywords == '') {
		...
		$description1_page = zyme_meta($post->ID, '_zyme_post_options', 'post_description');
	...
	<style>
		.zyme-certify {
			color: <?php echo cs_get_option('zyme_certify_color'); ?> !important;
		}

		#foot,
		#foot a {
			color: <?php echo cs_get_option('zyme_footer_color'); ?>;
		}

		.zyme-item,
		#header {
			opacity: <?php echo cs_get_option('zyme_opacity'); ?>;
		}

		<?php
		$zymebg = cs_get_option('zyme_background');
		if ($zymebg != null) {
			if (isset($zymebg["image"]) && $zymebg["image"] != '') { ?>body {
			background-image: url(<?php echo $zymebg["image"]; ?>);
			background-position: <?php echo $zymebg["position"]; ?>;
			background-repeat: <?php echo $zymebg["repeat"]; ?>;
			background-attachment: <?php if ($zymebg["attachment"] == '') echo 'scroll';
																	else echo $zymebg["attachment"]; ?>;
			background-size: <?php echo $zymebg["size"]; ?>;
		}

		<?php } else { ?>body {
			background: <?php echo $zymebg["color"]; ?>;
		}

		<?php }
		}
		$cardbg = cs_get_option('zyme_card_background');
		if ($cardbg != null) {
			if (isset($cardbg["image"]) && $cardbg["image"] != '') { ?>#sidebar .card-bg {
			...
			background-attachment: <?php if ($cardbg["attachment"] == '') echo 'scroll';
																	else echo $cardbg["attachment"]; ?>;
			background-size: <?php echo $cardbg["size"]; ?>;
		}

		<?php }
		}
		if (cs_get_option('zyme_user_css') != null) echo cs_get_option('zyme_user_css');
		?>
	</style>
</head>
<?php flush(); ?>
...
<body>
	<header id="header">
		<div id="pc-menu">
			<a id="menu-bar" class="zyme zyme-menu"></a>
            ...
            <?php if (is_user_logged_in()) {
							...
							echo '<i class="zyme zyme-login"></i>';
						} ?>
            </a>
			<?php if (is_user_logged_in()) { ?>
			<div id="personal-menu">
				<ul>
					<li><a href="<?php echo get_option('home'); ?>/wp-admin">后台&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="zyme zyme-dashboard"></i></a></li>
					<li><a href="<?php echo get_option('home'); ?>/wp-login.php?action=logout">登出&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="zyme zyme-logout"></i> </a></li>
				</ul>
		</div>
	<?php } ?>
	<form role="search" method="get" id="searchform" action="<?php echo home_url("/"); ?>" autocomplete="off">
		<i class="zyme zyme-search"></i>
