...
// 简化函数
function zyme_meta($ID, $op, $op_name) {
...
function password_protected_change( $content ) {
    ...
    if ( ! empty( $post->post_password ) && stripslashes( @$_COOKIE['wp-postpass_'.COOKIEHASH] ) != $post->post_password ) {
        $output = '
        <form action="' . esc_url( site_url( "wp-login.php?action=postpass", "login_post" ) ) . '" method="post" autocomplete="off"><p class="protected">这是一篇受密码保护的文章，您需要提供访问密码：</p><input name="post_password" class="input" type="password" size="20" /><input type="submit" name="Submit" class="button" value="' . __( "提交", "zyme" ) . '" /></form>
        ';
        return $output;
    } else {
        return $content;
...
// 喜欢功能
add_action('wp_ajax_nopriv_zyme_like', 'zyme_like');
add_action('wp_ajax_zyme_like', 'zyme_like');
function zyme_like(){
    global $wpdb,$post;
	$id = $_POST["zyme_id"];
	$action = $_POST["zyme_action"];
	if ( $action == 'zyme_like'){
    $zyme_raters = get_post_meta($id,'zyme_like',true);
    $expire = time() + 99999999;
    $domain = ($_SERVER['HTTP_HOST'] != 'localhost') ? $_SERVER['HTTP_HOST'] : false; 
    setcookie('zyme_like_'.$id,$id,$expire,'/',$domain,false);
    if (!$zyme_raters || !is_numeric($zyme_raters)) {
        update_post_meta($id, 'zyme_like', 1);
    }
    else {
            update_post_meta($id, 'zyme_like', ($zyme_raters + 1));
        }
    echo get_post_meta($id,'zyme_like',true);
    }
    die;
}

// 主循环中显示文章类型
function zyme_posts_per_page($query){
...
function create_shuoshuo() {
    $labels = array(
		'name'               => _x( '说说', 'zyme' ),
		'singular_name'      => _x( '说说', 'zyme' ),
        'add_new'            => _x( '新建说说', 'zyme' ),
        'add_new_item'       => __( '新建一个说说', 'zyme' ),
        'edit_item'          => __( '编辑说说', 'zyme' ),
        'new_item'           => __( '新说说', 'zyme' ),
        'all_items'          => __( '所有说说', 'zyme' ),
        'view_item'          => __( '查看说说', 'zyme' ),
        'search_items'       => __( '搜索说说', 'zyme' ),
        'not_found'          => __( '没有找到有关说说', 'zyme' ),
        'not_found_in_trash' => __( '回收站里面没有相关说说', 'zyme' ),
        'parent_item_colon'  => '',
        'menu_name'          => '说说'
    );
    $args = array(
...
add_action( 'add_meta_boxes', 'zyme_add_shuoshuo_box' );
function zyme_add_shuoshuo_box(){
    add_meta_box( 'zyme_shuoshuo_sticky', '置顶', 'zyme_shuoshuo_sticky', 'shuoshuo', 'side', 'high' );
}
function zyme_shuoshuo_sticky (){ ?>
...
add_action('pre_get_posts','zyme_posts_per_page');
...
// 阻止站内文章互相Pingback
function zyme_noself_ping( &$links ) { 
	$home = get_option( 'home' );
	foreach ( $links as $l => $link )
	if ( 0 === strpos( $link, $home ) )
	unset($links[$l]); 
}
add_action('pre_ping','zyme_noself_ping');

// 面包屑导航
function zyme_breadcrumbs() {
	$delimiter = '›'; // 分隔符
	...
	if ( !is_home() && !is_front_page() || is_paged() ) {
		...
		global $post;
		$homeLink = home_url();
		echo ' <a itemprop="breadcrumb" href="' . $homeLink . '">' . __( '首页' , 'zyme' ) . '</a> ' . $delimiter . ' ';
		if ( is_category() ) { // 分类 存档
			...
			echo $before ;
			printf( __( '"%s"的搜索结果：', 'zyme' ),  get_search_query() );
			echo  $after;
		} elseif ( is_tag() ) { //标签 存档
			echo $before ;
			printf( __( '标签: %s', 'zyme' ), single_tag_title( '', false ) );
			echo  $after;
		} elseif ( is_author() ) { // 作者存档
			global $author;
			$userdata = get_userdata($author);
			echo $before ;
			printf( __( '作者: %s', 'zyme' ),  $userdata->display_name );
			echo  $after;
		...
		if ( get_query_var('paged') ) { // 分页
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() )
				echo sprintf( __( '( 第%s页 )', 'zyme' ), get_query_var('paged') );
		}
		echo '</span>';
	}
}
...
// 文章摘要
function zyme_excerpt( $length, $more = '&hellip;', $echo = true ){
...
// 输出评论
function zyme_comment($comment, $args, $depth) {
...
?>
	<li class="comment-item" id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="commentator-comment" id="comment-<?php comment_ID(); ?>"><span class="commentator-name"><?php printf(__('<span class="author-name">%s</span> '), get_comment_author_link()); if ($comment->user_id == '1') { ?><i class="zyme zyme-certify"></i><?php } ?></span> <?php echo get_author_class($comment->comment_author_email,$comment->user_id); ?>
...
function zyme_protection_pre($commentdata){
...
// 自定义addComment
function zyme_disable_comment_js(){
    wp_deregister_script( 'comment-reply' );
}
add_action( 'init', 'zyme_disable_comment_js' );
...
// 文章归档
function zyme_archives_list() {
	if( !$output = get_option('zyme_archives_list') ){
        ...
        wp_reset_postdata();
        $output .= '</ul></li></ul></div>';
        update_option('zyme_archives_list', $output);
	}
    echo $output;
}
function clear_zyme_cache() {
    update_option('zyme_archives_list', ''); // 清空 zyme_archives_list
}
add_action('save_post', 'clear_zyme_cache'); // 新发表文章/修改文章时
...
<p style="padding-bottom: 15px;">(此邮件由系统自动发出, 请勿回复)</p></div></div></td></tr></tbody></table></div>';
		...
		if(cs_get_option( 'zyme_comment_reply' )==true) {
...
// 激活主题创建页面
function zyme_add_page($title,$slug,$page_template=''){
...
function zyme_add_pages() {
    global $pagenow;
    //判断是否为激活主题页面
    if ( 'themes.php' == $pagenow && isset( $_GET['activated'] ) ){
        zyme_add_page('归档','post-archives','post-archives.php');//页面标题、别名、页面模板
        zyme_add_page('友情链接','friend-link','friend-link.php');
    }
}
add_action( 'load-themes.php', 'zyme_add_pages' );