<?php 
if ( !function_exists( 'zyme_sidebar_posts_list' ) ) :
  /**
   * 边栏文章列表
   */
  function zyme_sidebar_posts_list( $query_args ){
...
if ( !function_exists( 'zyme_sidebar_posts_list_loop' ) ) :
  /**
   * 边栏文章列表样式
   */
  function zyme_sidebar_posts_list_loop(){
  ?>
    <li>
        <?php the_title( '<p class="post-title"><a href="' . esc_url( get_permalink() ) . '" title="' . the_title_attribute( 'echo=0' ) . '" rel="bookmark">「', '」</a></p>' ); ?>
      <p class="post-info"><span><i class="zyme zyme-view"></i>&nbsp;<?php echo getPostViews(get_the_ID()); ?></span><span><i class="zyme zyme-comment"></i>&nbsp;<?php if ( post_password_required() ) { echo '<a>密码保护</a>'; } else { comments_popup_link('0', '1', '%', '', '评论已关闭'); } ?></span><span><i class="zyme zyme-time"></i>&nbsp;<?php echo human_time_diff(get_the_time('U'), current_time('timestamp')) . '前'; ?></span></p>   
    </li>
  <?php
  }
endif;
...
// 最近访客
function zyme_most_active_friends($friends_num = 10) {
    global $wpdb;
...
function zyme_recent_photos($photos_num = 10) {
	$args = array(
